...
| `--<key!:value` | **Not Equals\*** | Block is **kept** if `key` exists AND does **NOT** equal `value`. |
| | | _Strict: If key missing, `!:` considers it not equal (kept)._ |

_> Note: Strict equality check. If `key` is missing, equivalence checks usually fail (block skipped)._
...
| `$key` | Same behavior. _(Note: `$1`, `$2` etc. are ignored)_     |
